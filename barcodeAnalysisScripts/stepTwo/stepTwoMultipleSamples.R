...
homeDirectory <- '/Volumes/YG_SCOPE_01/Temporary_VitoFiles/Analysis/stepTwo/'
sampleFolders = c('subset1/', 'subset2/', 'subset3/')
...
for(i in 1:numberSamples){
  stepTwoCellIDUMIBarcodes = as_tibble(read.table(paste0(homeDirectory, sampleFolders[i], 'stepTwoCellIDUMIBarcodes.txt'), stringsAsFactors=F, header = TRUE)) %>% mutate(sampleNum = 1)
